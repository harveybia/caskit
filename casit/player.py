    ...
    def step(self, state):
        ...
        if len(pending_orders) != 0:
            for order in pending_orders:
                path = nx.shortest_path(get_graph_without_orders(state, graph), station, order.get_node())
                orderCash = self.get_actual_gain(state, order, path)
                if (orderCash < 6):
                    continue
                selections[orderCash] = order
            opt_order = selections[self.key_with_max_val(selections)]

            #order = random.choice(pending_orders)
            path = nx.shortest_path(get_graph_without_orders(state, graph), station, opt_order.get_node())
